...
if(isset($_GET['delete-product'])){
    ...
    $query="delete from ecommerce_contactus where id='$id'";
    runQuery($query);
    header("Location:?m=Contact deleted successfully");
}
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<!--end::Svg Icon-->
												<input type="text" data-kt-ecommerce-category-filter="search" class="form-control form-control-solid w-250px ps-14" placeholder="Search Contacts" />
											</div>
										</div>
									...
									<div class="card-body pt-0">
										<table class="table align-middle table-row-dashed fs-6 gy-5 table-bordered" id="kt_ecommerce_category_table">
											...
											<tbody class="fw-bold text-gray-600">
											    ...
											    <?$products=getAll($con,"select * from ecommerce_contactus order by timeAdded desc");
											    foreach($products as $row){?>
											    
												<tr>
                                                    <td style="text-align: center;"><?echo $row['title']?></td>
                                                    <td style="text-align: center;"><?echo $row['name']?></td>
                                                    <td style="text-align: center;"><?echo $row['email']?></td>
													<td style="text-align: center;"><?echo date('Y-m-d H:i:s',$row['timeAdded'])?></td>
													<td>
                                                    <button class="btn btn-primary btn-sm message-toggle" data-bs-toggle="modal" data-bs-target="#message-modal-<?php echo $row['id']; ?>">View Message</button>
                                                    </td>

                                                    <!-- Modal -->
                                                                    <div class="modal fade" id="message-modal-<?php echo $row['id']; ?>" tabindex="-1" aria-labelledby="message-modal-<?php echo $row['id']; ?>-label" aria-hidden="true">
                                                                    <div class="modal-dialog modal-dialog-centered">
                                                                        <div class="modal-content">
                                                                        <div class="modal-header">
                                                                            <h5 class="modal-title" id="message-modal-<?php echo $row['id']; ?>-label">Message</h5>
