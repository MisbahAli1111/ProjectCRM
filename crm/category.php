...
if($logged==0)
    ...
    if(isset($_POST['categoryName'])){
        $cat = clear($_POST['categoryName']);
        $id=generateRandomString();
        $query="INSERT INTO `ecommerce_category`(`id`, `categories`) VALUES ('$id','$cat')";
        runQuery($query);
        header("Location:?m=Entry Added successfully");
    }    

if(isset($_GET['delete-service'])){
    $id = clear($_GET['delete-service']);
    $query="delete from ecommerce_category where id='$id'";
    runQuery($query);
									...
									<div class="card-body pt-0">
										<table class="table align-middle table-row-dashed fs-6 gy-5 table-bordered" id="kt_ecommerce_category_table">
											...
											<tbody class="fw-bold text-gray-600">
											    ...
											    <?$products=getAll($con,"select * from ecommerce_category");
											    foreach($products as $row){?>
											    
												<tr>
												    <td style="text-align: center;"><?echo $row['categories']?></td>
